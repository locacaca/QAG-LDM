...
AUDIO_OUTPUT_DIR="./batch_partial_gen_data"
MAIN_OUTPUT_DIR="./outputs_batch_partial_gen_quality"
...
echo "7. → 对质量控制分数 0.2, 0.5, 0.8 进行批量生成 (带重试机制)"
echo ""
echo "配置信息:"
echo "输入文件: ${INPUT_JSONL}"
echo "模型检查点: ${CKPT_PATH}"
echo "质量控制分数: ${QUALITY_SCORES[*]}"
...
# 步骤1: 处理merged.jsonl数据，生成任务配置 (已注释 - 数据处理已完成)
...
# # 检查Python脚本是否存在
# if [ ! -f "batch_partial_gen_processor.py" ]; then
#     echo "错误: Python脚本 batch_partial_gen_processor.py 不存在"
#     exit 1
# fi
...
# python batch_partial_gen_processor.py \
#     --input ${INPUT_JSONL} \
...
for QUALITY_SCORE in "${QUALITY_SCORES[@]}"; do
    ...
    while IFS= read -r line; do
        ...
        # 为当前任务创建输出目录
        TASK_OUTPUT_DIR="${QUALITY_OUTPUT_DIR}/${TRACK_UID}"
        ...
        echo "  ✓ 完成: ${TRACK_UID} (质量控制: ${QUALITY_SCORE})"
        echo ""
    ...
    echo "质量控制分数 ${QUALITY_SCORE} 的所有任务生成完成！"
...
echo "✓ 批量生成: 完成质量控制分数 ${QUALITY_SCORES[*]} 的生成 (带重试机制)"
echo ""
echo "输出目录: ${MAIN_OUTPUT_DIR}"
echo "处理的任务数量: ${TASK_COUNT}"
echo "质量控制分数: ${QUALITY_SCORES[*]}"
echo "总共生成了 $((TASK_COUNT * ${#QUALITY_SCORES[@]})) 个音频文件"
echo ""
...
8. ✓ 批量生成质量控制分数 0.2, 0.5, 0.8 (带重试机制)
...
质量控制分数:
$(for score in "${QUALITY_SCORES[@]}"; do echo "- ${score}"; done)

输出目录结构:
${MAIN_OUTPUT_DIR}/
$(for score in "${QUALITY_SCORES[@]}"; do echo "├── quality_${score}/"; done)
$(for score in "${QUALITY_SCORES[@]}"; do echo "│   ├── test_Track01892/"; done)
$(for score in "${QUALITY_SCORES[@]}"; do echo "│   ├── test_Track02029/"; done)
$(for score in "${QUALITY_SCORES[@]}"; do echo "│   └── ..."; done)
...
实验目的:
验证质量感知模型在部分生成任务中是否能够根据不同的质量分数
生成相应质量的音频，以及在不同未知音轨类型下质量感知能力的一致性。
...
echo "- 质量控制分数: ${#QUALITY_SCORES[@]} 个 (${QUALITY_SCORES[*]})"
echo "- 总生成文件数: $((TASK_COUNT * ${#QUALITY_SCORES[@]}))"
echo "- 输出目录: ${MAIN_OUTPUT_DIR}"
echo "- 汇总报告: ${SUMMARY_FILE}"
echo ""
echo "🎉 批量质量控制部分生成完成！"
echo "   现在您可以开始分析不同质量控制分数的生成效果了！"
