...
def create_training_mgeldm_from_config(
    ...
    logging_config = dict(training_config["logging_config"])
    checkpoint_config = training_config.get("checkpoint", {})
    if logging_config.get("attention_sink_enabled", False) and "every_n_train_steps" in checkpoint_config:
        logging_config["attention_sink_record_interval"] = checkpoint_config["every_n_train_steps"]
    ...
    return MGELDM_TrainingWrapper(
        ...
        quality_aux_loss_weight=training_config.get("quality_aux_loss_weight", 0.1),
