...
from .attention_sink_logger import create_attention_logger, set_attention_logger, get_attention_logger
...
class MGELDM_TrainingWrapper(pl.LightningModule):
    ...
    def __init__(
        ...
        quality_aux_loss_weight: float = 0.1,
        ...
        self.quality_aux_loss_weight = quality_aux_loss_weight
        ...
        # Attention Sink Logger 配置
        self.attention_sink_log_dir = logging_config.get("attention_sink_log_dir", "./logs/attention_sink")
        self.attention_sink_record_interval = logging_config.get("attention_sink_record_interval", 100)
        self.attention_sink_enabled = logging_config.get("attention_sink_enabled", False)
        
        print(f"[DEBUG] attention_sink_enabled={self.attention_sink_enabled}")
        print(f"[DEBUG] attention_sink_record_interval={self.attention_sink_record_interval}")
        print(f"[DEBUG] attention_sink_log_dir={self.attention_sink_log_dir}")
        
        self.attention_logger = None
        if self.attention_sink_enabled:
            print(f"[DEBUG] _setup_attention_logger queued, will run after self.diffusion is set")
        ...
        # Attention Sink Logger: 必须在 self.diffusion 设置后调用
        if self.attention_sink_enabled:
            self._setup_attention_logger()
    ...
    def _setup_attention_logger(self):
        """初始化 Attention Sink Logger"""
        from ..models.transformer import ContinuousTransformer
        
        print(f"[DEBUG] _setup_attention_logger called")
        
        # 获取 transformer 深度
        num_layers = 16  # 默认值，可从模型配置获取
        
        # 创建 logger
        self.attention_logger = create_attention_logger(
            log_dir=self.attention_sink_log_dir,
            record_interval=self.attention_sink_record_interval,
        ...
        # 将 logger 绑定到 transformer
        bound_count = 0
        for module in self.diffusion.model_dit.modules():
            if isinstance(module, ContinuousTransformer):
                module.set_attention_logger(self.attention_logger)
                bound_count += 1
                print(f"[AttentionSinkLogger] Bound to ContinuousTransformer")
        
        print(f"[DEBUG] Total ContinuousTransformer bound: {bound_count}")
        
        set_attention_logger(self.attention_logger)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Attention Sink Logger: 更新 step 计数器
        if self.attention_logger is not None:
            self.attention_logger.step()
            if self.attention_logger.step_count % 1000 == 0:
                print(f"[DEBUG] attention_logger.step() called, step_count={self.attention_logger.step_count}")
        
        # with torch.cuda.amp.autocast():
        self.diffusion.conditioner.set_device(self.device)
        
        # 随机丢弃cocola_score（10%概率）
        if "cocola_score" in batch and torch.rand(1).item() < 0.1:
            # 保存原始值以便恢复
            original_cocola = batch["cocola_score"]
            # 临时设置为None
            batch["cocola_score"] = None
            conditioning = self.diffusion.conditioner(batch)
            # 恢复原始值
            batch["cocola_score"] = original_cocola
        else:
            conditioning = self.diffusion.conditioner(batch)
        """
        conditioning:
        ...
        # with torch.cuda.amp.autocast():
        should_record_attention = self.attention_logger is not None and self.attention_logger.should_record()

        diffusion_output = self.diffusion(
            noised_inputs,
            t_,
            cond=conditioning,
            cfg_dropout_prob=self.cfg_dropout_prob,
            return_info=should_record_attention,
            return_quality_aux=True,
        )
        if self.attention_logger is not None and should_record_attention:
            self.attention_logger.clear_forced_record()

        quality_aux = None
        if isinstance(diffusion_output, tuple):
            if len(diffusion_output) == 3:
                output, _, quality_aux = diffusion_output
            elif len(diffusion_output) == 2:
                output, quality_aux = diffusion_output
            else:
                output = diffusion_output[0]
        else:
        ...
        if "cocola_score" in batch:
            cocola_scores = batch["cocola_score"]
            cocola_scores_raw = batch.get("cocola_score_raw", None)  # 获取原始值，如果存在
            
            if cocola_scores is not None and not all(score is None for score in cocola_scores):
                valid_scores = [score for score in cocola_scores if score is not None]
                if valid_scores:
                    avg_cocola = sum(valid_scores) / len(valid_scores)
                    hidden_log_dict = {
                        "train/cocola_batch_min": min(valid_scores),  # 批次最小值
                        "train/cocola_batch_max": max(valid_scores),  # 批次最大值
                    }
                    visible_log_dict = {
                        "train/cocola_batch_avg": avg_cocola,  # 更清晰的命名，表明这是批次平均值
                    }
                    if hidden_log_dict:
                        self.log_dict(hidden_log_dict, prog_bar=False, on_step=True)
                    if visible_log_dict:
                        self.log_dict(visible_log_dict, prog_bar=True, on_step=True)
                    
                    # 记录cocola分数，但不输出调试信息
        
        loss, losses = self.losses(loss_info)
        if quality_aux is not None:
            quality_mask = quality_aux["quality_score_mask"]
            if quality_mask.any():
                quality_aux_loss = F.mse_loss(
                    quality_aux["quality_score_pred"][quality_mask],
                    quality_aux["quality_score_target"][quality_mask],
                )
                loss = loss + self.quality_aux_loss_weight * quality_aux_loss
                losses["quality_aux_loss"] = quality_aux_loss

        if self.attention_logger is not None:
            self.attention_logger.record_loss(loss, losses)
    ...
    def on_train_start(self):
        """训练开始时不预增 step，保证记录步与 checkpoint 保存步对齐。"""
        return

    def on_train_epoch_start(self):
        """每个 epoch 开始时强制在首个 batch 记录一次 attention。"""
        if self.attention_logger is not None:
            self.attention_logger.request_record_next_step()
    
    def on_train_epoch_end(self):
        """每个 epoch 结束时保存 attention sink 数据"""
        if self.attention_logger is not None:
...
class MGELDM_DemoCallback(pl.Callback):
    ...
    # on_train_epoch_end 方法已移除（由 MGELDM_TrainingWrapper 处理)
