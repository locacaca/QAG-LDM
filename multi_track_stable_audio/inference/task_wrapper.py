...
class InferenceTaskWrapper:
    ...
    def total_mixture_generation(
        ...
        quality_score: tp.Optional[float] = None, ## 质量控制分数，范围0-1，None表示禁用质量控制
        verbose: bool=False,
        ...
        # ===== Cross-attention: convert quality scores to tensor =====
        # DiT模型期望quality_scores是torch.Tensor，不是float
        quality_scores_tensor = None
        if quality_score is not None:
            quality_scores_tensor = torch.tensor([quality_score] * bs, dtype=torch.float32, device=self.device)

        # 处理质量控制（仅在quality_score不为None时）
        # 注意：当启用 cross-attention 时，质量控制通过 quality_scores 参数注入
        # 推理时这里可以保留简单的加权融合作为兼容
    ...
    def partial_generation_iterative(
        ...
        num_timesteps: int=100,
        repaint_n: int=1, ## Reapinting strategy for outpainting
        quality_score: float=0.5, ## 质量控制分数，范围0-1
        verbose: bool=False,
    ):
        """
    ...
    def generate_given_track(
        ...
        if raw_quality_score is not None:
            quality_scores_tensor = torch.tensor([raw_quality_score] * bs, dtype=torch.float32, device=self.device)
